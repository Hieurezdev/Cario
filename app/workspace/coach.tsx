...
function MessageBody({ message }: { message: Message }) {
  if (message.speaker === "user") return <p>{message.text}</p>;
  return <ReactMarkdown remarkPlugins={[remarkGfm]} skipHtml>{message.text}</ReactMarkdown>;
}
  ...
  async function sendQuestion(question: string) {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/coach/reply`, {
        ...
        body: JSON.stringify({ question: cleaned, interest, goal, evidence_count: evidenceCount, history: messages.slice(-12).map((message) => ({ role: message.speaker === "user" ? "user" : "assistant", content: message.text })) }),
      });
