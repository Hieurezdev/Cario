...
export default function Discover({ careers, profile, update }: DiscoveryProps) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (step === 4) {
      ...
      const matches = prompts[4].options.filter((option) => entered.includes(normalize(option)));
      if (draft.trim() && matches.length === 0) {
        setError("Mình chưa nhận ra kỹ năng đó trong danh sách. Hãy chọn gợi ý bên trên hoặc nhập tên kỹ năng được liệt kê.");
        return;
      }
  ...
  return <section className="ws-discovery-page" aria-labelledby="discovery-title">
    ...
    <div className="ws-discovery-thread">
      ...
      {complete ? <div className="ws-discovery-finish" ref={latestMessage} aria-live="polite"><span className="ws-discovery-avatar" aria-hidden="true">✳</span><div><p className="ws-eyebrow">GỢI Ý TỪ CARIO</p><h2>Một điểm bắt đầu, chưa phải kết luận.</h2><p>Bạn đang quan tâm đến <strong>{profile.interest}</strong>{profile.strength && <>, nhận thấy mình có thế mạnh <strong>{profile.strength.toLocaleLowerCase("vi")}</strong></>}{profile.value && <> và coi trọng việc <strong>{profile.value.toLocaleLowerCase("vi")}</strong></>}. Hãy thử một công việc nhỏ trước khi chọn hướng đi dài hơn.</p>{suggested.length > 0 ? <div className="ws-discovery-next"><span>HƯỚNG CÓ THỂ TÌM HIỂU</span>{suggested.map((career) => <div key={career.id}><h3>{career.title}</h3><p>{career.description}</p></div>)}<Link className="ws-link" href="/workspace/compare">So sánh các hướng nghề →</Link></div> : <div className="ws-discovery-next"><span>BƯỚC TIẾP THEO</span><p>Chưa có nghề trong danh sách thuộc lĩnh vực này. Hãy bắt đầu bằng một thử thách để hiểu rõ hơn điều khiến bạn hứng thú.</p><Link className="ws-link" href="/workspace/quests">Xem thử thách nghề →</Link></div>}</div></div> : <div className="ws-discovery-current" ref={latestMessage} aria-live="polite"><span className="ws-discovery-avatar" aria-hidden="true">✳</span><div className="ws-discovery-prompt"><span>CARIO</span><h2>{prompts[step].question}</h2><p>{prompts[step].detail}</p><div className="ws-discovery-options">{prompts[step].options.map((option) => <button type="button" key={option} className={step === 4 && skills.includes(option) ? "selected" : ""} aria-pressed={step === 4 ? skills.includes(option) : undefined} onClick={() => step === 4 ? setSkills((current) => current.includes(option) ? current.filter((skill) => skill !== option) : [...current, option]) : answer(option)}>{option}</button>)}</div>{step === 4 ? <div className="ws-discovery-prompt-actions"><button type="button" className="ws-button dark" onClick={() => finish(skills)}>Xem kết quả ↗</button><button type="button" onClick={() => finish([])}>Chưa có kỹ năng nào</button></div> : step === 2 || step === 3 ? <button type="button" className="ws-discovery-skip" onClick={() => answer("Mình chưa rõ")}>Mình chưa rõ →</button> : null}</div></div>}
    </div>
    <div className="ws-discovery-entry">{complete ? <button type="button" className="ws-discovery-restart" onClick={restart}>Khám phá lại từ đầu ↗</button> : <><form className="ws-coach-composer" onSubmit={submit}><label className="sr-only" htmlFor="discovery-draft">Câu trả lời của bạn</label><input id="discovery-draft" value={draft} onChange={(event) => { setDraft(event.target.value); setError(""); }} placeholder={step === 4 ? "Nhập kỹ năng, ngăn cách bằng dấu phẩy..." : "Chọn gợi ý hoặc nhập câu trả lời..."} /><button type="submit" aria-label="Gửi câu trả lời" disabled={!draft.trim() && !(step === 4 && skills.length > 0)}>↑</button></form>{error && <p className="ws-discovery-error" role="alert">{error}</p>}<p>Chọn một gợi ý hoặc nhập câu trả lời. Thông tin được lưu trên trình duyệt này.</p></>}</div>
  </section>;
}
