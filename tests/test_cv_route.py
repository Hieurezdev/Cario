...
class CvRouteTests(unittest.TestCase):
    ...
    def test_missing_key_is_actionable(self) -> None:
        ...
        self.assertIn("Qwen proxy", caught.exception.detail)

    def test_generation_failure_is_not_hidden_as_unavailable(self) -> None:
        with patch.object(settings, "qwen_api_key", "test-key"), patch.object(settings, "qwen_base_url", "http://proxy.test/v1"), patch(
